...
export default function ImageCard({ post, index, refetch }) {
    ...
    useEffect(() => {
        setIsLiked(post?.likes?.some(like => like.username === userData?.loggedInUser?.username))
    }, [post?.likes, userData?.loggedInUser?.username])
